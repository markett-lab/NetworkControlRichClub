...
%% add path to NCT toolbox; define some dataset-specific variables

% THIS SCRIPT ADDAPTS NCT_MAIN.m TO WORK ON TOY DATA PROVIDED WITH THE CODE

% you just need to adapt these two lines, then can execute the script and
% it will output full NCT analyses (global, regional, subset,
% subset_rotated) on the toy data provided and output it into toy_data
...
data_dir = 'toy_data';
% toy data contains random data for 3 subjects named 123, 456 and 789

% output directory
out_dir = 'toy_data'; % in this case saving output in toy data folder

% load subj IDs
sub_string = fileread('toy_data/subj_ID_list.txt');
...
glob_reg = 1;
outname_global_regional = 'example_output_glob_reg.mat';

% for analysis of role of a subset of regions, set this to 1:
subset = 1;
outname_subset = 'example_output_subset.mat';
subset_ids = [2 5 7 12 15 19]; 

rotated_subset = 1;
% for null distribution, set to 1 and uncomment following two lines:
load([working_dir '/' data_dir '/50_rotated_lausanne250.mat']);
outname_rotated = 'example_output_rotated.mat';
